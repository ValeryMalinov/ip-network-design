<template>
  <g>
    <g v-if="points && points.length" @mouseenter="mouseEnter" @mouseleave="mouseLeave">
      <g>
        <line
    ...
    <g v-else @mouseenter="mouseEnter" @mouseleave="mouseLeave">
      <g>
        <path :d="curve" :style="largeStrokeStyle" stroke-width="8" fill="none"/>
        <path :d="curve" style="stroke:rgb(0,0,0);" stroke-width="2" fill="none"/>
      </g>
    </g>
  </g>
</template>
...
export default {
  ...
  computed: {
    ...
    curve() {
      var x1 = Math.trunc(this.positionFrom.x - 4),
